...
def create_drug_dict():
    raw_paths = '/home/yurui/GDSC_2/GDSC2_dataset/meta_data/drug_meta.csv'
    dataset = pd.read_csv(raw_paths)
    dataset.set_index('DRUG_ID', inplace=True)
    SMILES_CHARS = []
    ...
    def smiles_encoder(smiles, maxlen=160):
        ...
        for i, c in enumerate(smiles):
            X[smi2index[c], i] = 1
        return X
    # maxlen = max([len(smiles) for smiles in dataset.SMILES]) >> 122
    maxlen = 122
    ...
    drug_bond_dict = {}
    problem_drug = []
    for drug_id, (smiles, name, PATHWAY_NAME, PATHWAY_TYPE) in tqdm(dataset.iterrows()):
        try:
            ...
            drug_atom.drug_name = name
            drug_atom.PATHWAY_TYPE = PATHWAY_TYPE
            drug_bond.drug_name = name
            ...
            print(f'The drug {drug_id} has problem, the smiles is {smiles}')
            problem_drug.append(drug_id)
            continue
    print('Finish generating drugs')
    def save_drug_feat():
        save_path = '../GDSC2_dataset/Drug_feature/'
    ...
    save_drug_feat()
    dataset = dataset[~dataset.index.isin(problem_drug)]
    dataset.to_csv('/home/yurui/GDSC_2/GDSC2_dataset/meta_data/drug_meta.csv')
    return drug_atom_dict,drug_bond_dict
def load_drug_feat():
    drug_atom_feat = np.load('/home/yurui/GDSC_2/GDSC2_dataset/Drug_feature/drug_feat_atom.npy',allow_pickle=True).item()
    drug_bond_feat = np.load('/home/yurui/GDSC_2/GDSC2_dataset/Drug_feature/drug_feat_bond.npy',allow_pickle=True).item()
    print('finish loading drug data!')
    return drug_atom_feat, drug_bond_feat

if __name__ =='__main__':
    drug_feat_path = '/home/yurui/GDSC_2/GDSC2_dataset/Drug_feature/drug_feat_atom.npy'
    if not os.path.exists(drug_feat_path):
